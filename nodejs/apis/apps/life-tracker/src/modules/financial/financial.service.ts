...
export class FinancialService {
  constructor(
    ...
    @InjectModel(FinancialGoal.name)
    private financialGoalModel: Model<FinancialGoal>,
  ) {}
      ...
      if (!portfolio || portfolio.assets.length === 0) {
        return {
          ...
          data: {
            ...
            sharpeRatio: 0,
            recommendations: [
              'Adicione ativos ao seu portfólio para análise de risco',
            ],
          },
          timestamp: new Date().toISOString(),
        };
  ...
  async addAsset(
    ...
    try {
      ...
      const currentPrice =
        assetData.averagePrice * (1 + (Math.random() - 0.5) * 0.2); // ±10% variação
      ...
      return {
        ...
        data: {
          ...
          profitLoss:
            (newAsset.currentPrice - newAsset.averagePrice) * newAsset.quantity,
          profitLossPercentage:
            newAsset.averagePrice > 0
              ? ((newAsset.currentPrice - newAsset.averagePrice) /
  ...
  async updateAsset(
    ...
    try {
      ...
      if (assetData.symbol)
        portfolio.assets[assetIndex].symbol = assetData.symbol;
      if (assetData.quantity)
        portfolio.assets[assetIndex].quantity = assetData.quantity;
      if (assetData.averagePrice)
        portfolio.assets[assetIndex].averagePrice = assetData.averagePrice;
      ...
      return {
        ...
        data: {
          ...
          profitLoss:
            (updatedAsset.currentPrice - updatedAsset.averagePrice) *
            updatedAsset.quantity,
          profitLossPercentage:
            updatedAsset.averagePrice > 0
              ? ((updatedAsset.currentPrice - updatedAsset.averagePrice) /
                  updatedAsset.averagePrice) *
                100
              : 0,
  ...
  async getFinancialGoal(
    ...
    try {
      const goal = await this.financialGoalModel
  ...
  async getPriceHistory(
    ...
    try {
      ...
      const days =
        period === '1d' ? 1 : period === '1w' ? 7 : period === '1m' ? 30 : 90;
