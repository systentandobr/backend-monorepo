...
let FinancialService = class FinancialService {
    ...
    async getPortfolioRiskAnalysis(userId) {
        try {
            const portfolio = await this.portfolioModel.findOne({ userId }).exec();
    ...
    async addAsset(userId, assetData) {
        try {
            ...
            const currentPrice = assetData.averagePrice * (1 + (Math.random() - 0.5) * 0.2);
            const newAsset = {
    ...
    async updateAsset(userId, assetId, assetData) {
        try {
            const portfolio = await this.portfolioModel.findOne({ userId }).exec();
            if (!portfolio) {
                return {
    ...
    async getFinancialGoal(userId, id) {
        try {
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    createdAt: savedGoal.createdAt || new Date().toISOString(),
                },
                timestamp: new Date().toISOString(),
            };
        }
    ...
    async updateFinancialGoal(userId, id, goalData) {
        try {
            const goal = await this.financialGoalModel.findOneAndUpdate({ _id: id, userId }, {
    ...
    async deleteFinancialGoal(userId, id) {
        try {
