...
export class OrdersService {
  ...
  async create(
    ...
    if (createOrderDto.referralCode) {
      try {
        const referral = await this.referralsService.validateCode(
          createOrderDto.referralCode,
        );
        referralId = new Types.ObjectId(referral.id);
      } catch (error) {
        this.logger.warn(
          `Código de indicação inválido: ${createOrderDto.referralCode}`,
          error.message,
        );
    ...
    this.notificationsService
      ...
      .catch((err) => {
        this.logger.error(
          `Erro ao enviar notificação de novo pedido: ${err.message}`,
        );
      });
  ...
  async updateStatus(
    ...
    const order = await this.orderModel
      .findOneAndUpdate({ _id: id, unitId }, updateData, { new: true })
      .exec();
    ...
    if (
      ...
      try {
        ...
        this.logger.error(
          `Erro ao processar indicação do pedido ${id}:`,
          error.message,
        );
    ...
    if (updateDto.status === 'cancelado' && order.referralId) {
      try {
        ...
        this.logger.debug(
          `Pedido ${id} cancelado, recompensas devem ser canceladas`,
        );
      } catch (error) {
        this.logger.error(
          `Erro ao cancelar recompensas do pedido ${id}:`,
          error.message,
        );
      }
    }
