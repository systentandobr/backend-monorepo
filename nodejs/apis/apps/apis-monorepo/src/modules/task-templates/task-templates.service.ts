...
import { TaskTemplate, TaskTemplateDocument } from './schemas/task-template.schema';
...
export class TaskTemplatesService {
    ...
    async create(createTaskTemplateDto: CreateTaskTemplateDto): Promise<TaskTemplate> {
    ...
    async findDefaults(): Promise<TaskTemplate[]> {
        return this.taskTemplateModel.find({ isDefault: true }).sort({ order: 1 }).exec();
    }

    async update(id: string, updateTaskTemplateDto: UpdateTaskTemplateDto): Promise<TaskTemplate> {
