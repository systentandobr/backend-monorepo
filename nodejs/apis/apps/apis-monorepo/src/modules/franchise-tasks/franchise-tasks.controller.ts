...
export class FranchiseTasksController {
    ...
    @Patch(':id')
    update(@Param('id') id: string, @Body() updateFranchiseTaskDto: UpdateFranchiseTaskDto) {
        return this.franchiseTasksService.update(id, updateFranchiseTaskDto);
    }

    @Post(':id/complete-step')
    completeStep(@Param('id') id: string, @Body() completeTaskStepDto: CompleteTaskStepDto) {
