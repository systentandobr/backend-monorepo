...
import { FranchiseTask, FranchiseTaskDocument } from './schemas/franchise-task.schema';
...
export class FranchiseTasksService {
    constructor(
        @InjectModel(FranchiseTask.name) private franchiseTaskModel: Model<FranchiseTaskDocument>,
        private readonly taskTemplatesService: TaskTemplatesService,
    ) { }

    async create(createFranchiseTaskDto: CreateFranchiseTaskDto): Promise<FranchiseTask> {
    ...
    async findAllByFranchise(franchiseId: string): Promise<FranchiseTask[]> {
        return this.franchiseTaskModel.find({ franchiseId }).sort({ createdAt: 1 }).exec();
    }

    async findAllByUser(userId: string): Promise<FranchiseTask[]> {
        return this.franchiseTaskModel.find({ userId }).sort({ createdAt: 1 }).exec();
    }
    ...
    async update(id: string, updateFranchiseTaskDto: UpdateFranchiseTaskDto): Promise<FranchiseTask> {
    ...
    async completeStep(id: string, completeTaskStepDto: CompleteTaskStepDto): Promise<FranchiseTask> {
        const task = await this.findOne(id);

        // Check if step is already completed
        const existingStep = task.completedSteps.find(s => s.stepOrder === completeTaskStepDto.stepOrder);
        if (existingStep) {
        ...
        return this.franchiseTaskModel.findByIdAndUpdate(
            id,
    ...
    async initializeDefaults(franchiseId: string, userId: string): Promise<FranchiseTask[]> {
